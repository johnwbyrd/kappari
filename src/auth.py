...
class Auth:
    ...
    def decrypt_license_data(self):
        ...
        self.license_data = self._decrypt_data(encrypted_data, self.config.purchase_data_key)
        ...
        signature_raw = self._decrypt_data(encrypted_signature, self.config.purchase_signature_key)
    ...
    def authenticate(self, email, password):
        ...
        jwt_token = self.client.authenticate(email, password, self.license_data, self.signature)
        
        if jwt_token:
            log.debug("JWT Token received: %s", jwt_token[:20] + "..." if len(jwt_token) > 20 else jwt_token)
...
def main():
    ...
    try:
        ...
        log.info("  Purchase Date: %s", license_json.get('purchase_date', 'N/A'))
            ...
            if jwt_token:
                ...
                if response and response.status_code == 200:
                    ...
                    if auth.config.pretty_json:
                        log.debug("Response: %s", json.dumps(response.json(), indent=2))
                    else:
                        log.debug("Response: %s", response.json())
                else:
                    log.error("API request failed: %s", response.status_code if response else 'No response')
            else:
                log.error("Authentication failed")
        else:
            log.warning("Credentials not found in config")
            log.info("Set KAPPARI_EMAIL and KAPPARI_PASSWORD in your .env file to test authentication")
