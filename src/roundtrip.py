...
def encrypt_paprika_data(plaintext: str, password: str, salt: bytes = None) -> Tuple[str, bytes]:
    """
...
def decrypt_paprika_data(encrypted_b64: str, password: str) -> Tuple[str, bytes]:
    """
...
def perform_round_trip_test():
    ...
    try:
        encrypted_data_original, encrypted_signature_original = read_encrypted_data_from_db(config.db_path)
        log.info("Successfully read encrypted data")
        log.info("  Data length: %d characters", len(encrypted_data_original))
        log.info("  Signature length: %d characters", len(encrypted_signature_original))
    except Exception as e:
    ...
    try:
        ...
        log.info("  Email: %s***", license_json.get('email', 'N/A')[:3] if license_json.get('email') else 'N/A')
        log.info("  License: %s***", license_json.get('key', 'N/A')[:5] if license_json.get('key') else 'N/A')
    ...
    try:
        decrypted_signature, sig_salt = decrypt_paprika_data(
            encrypted_signature_original,
            config.purchase_signature_key
        )
        log.info("Successfully decrypted signature")
        log.info("  Signature preview: %s...", decrypted_signature[:30] if len(decrypted_signature) > 30 else decrypted_signature)
    ...
    try:
        ...
        log.info("  Re-encrypted length: %d characters", len(re_encrypted_data))
    ...
    try:
        ...
        log.info("  Re-encrypted length: %d characters", len(re_encrypted_signature))
    ...
    if data_matches:
        ...
        log.debug("  Original first 50 chars: %s", encrypted_data_original[:50])
    ...
    if signature_matches:
        ...
        log.debug("  Original first 50 chars: %s", encrypted_signature_original[:50])
        log.debug("  Re-encrypted first 50 chars: %s", re_encrypted_signature[:50])
...
def main():
    ...
    if not os.path.exists(config.db_path):
        ...
        log.info("\nTo use this script:")
        log.info("1. Ensure Paprika 3 is installed and has been run at least once")
        log.info("2. Set KAPPARI_DB_PATH in your .env file to point to Paprika.sqlite")
